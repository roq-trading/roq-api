...
inline std::uint32_t atomic_acquire(const std::uint32_t *result) {
  return __atomic_load_4(result, static_cast<int>(Atomic::ACQUIRE));
}

inline std::uint64_t atomic_acquire(const std::uint64_t *result) {
  return __atomic_load_8(result, static_cast<int>(Atomic::ACQUIRE));
}
