...
struct ROQ_PUBLIC GatewaySettings final {
  ...
  double mbp_tick_size_multiplier =
      std::numeric_limits<double>::quiet_NaN();  //!< MBP multiplier used to manage prices as integer
  double mbp_min_trade_vol_multiplier =
      std::numeric_limits<double>::quiet_NaN();  //!< MBP multiplier used to manage quantities as integer
